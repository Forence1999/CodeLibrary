...
def normalize_sentence(sentence):
	...
	global clean  # whether to delete sentence containing special characters (except letters, apostrophes and spaces)
	
	sentence = sentence.replace("<unk>", " ").replace("<UNK>", " ")
	assert "<" not in sentence and ">" not in sentence
	
	if clean and not re.match(r"^[a-zA-Z ']*$", sentence):
		return ""
...
def txt2tsv_wrd_ltr(txt_path, tsv_path, wrd_path, ltr_path, flac_dsroot):
	'''
	convert kaldi text to fairseq *.tsv *.wrd and *.ltr
	...
	segments = [i.split(maxsplit=1) for i in segments if i.strip() != ""]
	data = [(flac_dict[i[0]], i[1]) for i in segments if len(i) == 2]  # list of (path, sentence)
...
# 	# dst_path.write_text("\n".join(line_ls) + "\n")
#
# 	sentences = [line[1] for line in line_ls if len(line) > 1]
# 	char_set = set()
# 	for sentence in sentences:
# 		sentence = sentence.replace("<unk>", " ").replace("<UNK>", " ")
# 		assert "<" not in sentence and ">" not in sentence
# 		char_set.update(list(sentence))
#
# 	print(f"{char_set}\n contains {len(char_set)} characters")


flac_dsroot = Path('/data2/swang/asr/TEDLIUM_release-3/flac')
clean = True  # whether to delete sentence containing special characters (except letters, apostrophes and spaces)
...
if __name__ == '__main__':
	...
	for dataset_split in dataset:
		...
		data_dir = Path(f'./{dataset_split}').absolute()
		processed_dir = Path(f'./processed').absolute()
		
		text_path = Path(f'./{dataset_split}/text').absolute()
		
		# generate tsv wrd and ltr files
		if clean:
			txt2tsv_wrd_ltr(text_path,
							processed_dir / 'clean' / f'{dataset_split}.tsv',
							processed_dir / 'clean' / f'{dataset_split}.wrd',
							processed_dir / 'clean' / f'{dataset_split}.ltr',
							flac_dsroot=flac_dsroot / "legacy" / dataset_split.rstrip(".orig") / "sph")
		else:
			txt2tsv_wrd_ltr(text_path,
							processed_dir / 'noisy' / f'{dataset_split}.tsv',
							processed_dir / 'noisy' / f'{dataset_split}.wrd',
							processed_dir / 'noisy' / f'{dataset_split}.ltr',
							flac_dsroot=flac_dsroot / "legacy" / dataset_split.rstrip(".orig") / "sph")
